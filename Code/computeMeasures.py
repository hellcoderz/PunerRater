# computes the entropy and distinctiveness of P(m | w) using
...
useConfusability = bool(int(sys.argv[3]))
self_relatedness = float(sys.argv[4])
scaling_parameter = float(sys.argv[5])
...
if useTrigrams:
    ngramType = "trigram"
if useConfusability:
    confusedType = "_confused"

filename = "../ModelOutputs/" + punType + "_" + ngramType + confusedType + "_" + str(int(self_relatedness)) + "_" + str(int(scaling_parameter)) + ".csv"
print filename
writeFile = open(filename, "wr")

# The output is formatted as follows:
writeFile.write("punType,sentenceID,sentenceType,sum_m1_ngram,sum_m2_ngram,sum_m1_relatedness,sum_m2_relatedness,p_m1_given_w,p_m2_given_w,entropy,m1KL,m2KL,m1Focus,m2Focus\n")
...
# confusability dictionary
confusedFile = open("../ProcessedData/confusability_ratings_" + punType + ".csv", "r")
confusedDict = dict()

firstline = 0
for l in confusedFile:
    if firstline == 0:
        firstline = 1
    else:
        l = l.strip()
        toks = l.split(",")
        homophoneID = toks[0]
        confusedScore = float(toks[1])
        confusedDict[homophoneID] = confusedScore
...
for l in pairFile:
    if firstLine == 0:
        ...
        if sentenceID not in sentenceDict:
            ...
            if useConfusability:
                m2RelatednessArray = [confusedDict[homophoneID]]
            else:
...
for k, v in sentenceDict.iteritems():
    ...
    writeFile.write(punType + "," + sentenceID + "," + sentenceType + "," + str(sum(m1Ngram)) + "," + str(sum(m2Ngram)) + "," + str(sum(m1Relatedness)) + "," + str(sum(m2Relatedness)) + "," + str(probM1) + "," + str(probM2) + "," + str(entropy) + "," + str(KL1) + "," + str(KL2) + "," + ";".join(maxM1FocusWords) + "," + ";".join(maxM2FocusWords)+"\n")  #print "P(m1|w): " + str(probM1) + "; P(m2|w): " + str(probM2)
